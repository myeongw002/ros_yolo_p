...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    if color_seg.shape[:2] != img.shape[:2]:
    ...
    # Assuming `color_seg` is the resized segmentation mask
    resized_da_mask = (color_seg[..., ::-1] == [0, 255, 0]).all(axis=2)
    resized_ll_mask = (color_seg[..., ::-1] == [255, 0, 0]).all(axis=2)

    # Get all pixel coordinates that belong to drivable areas in the resized image
    resized_da_pixel_coords = np.argwhere(resized_da_mask)
    # Get all pixel coordinates that belong to lanes in the resized image
    resized_ll_pixel_coords = np.argwhere(resized_ll_mask)
    
    #print("Resized Drivable Area Pixels:", resized_da_pixel_coords)
    #print("Resized Lane Pixels:", resized_ll_pixel_coords)
    ...
    return img , resized_da_pixel_coords, resized_ll_pixel_coords
